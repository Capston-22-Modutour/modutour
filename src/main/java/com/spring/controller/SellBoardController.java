...
public class SellBoardController {
	...
	public String postSellWrite(BoardDTO dto) throws Exception {
	...
	public void getSellView(@RequestParam("sell_bno") int sell_bno, Model model) throws Exception {
		...
		reply = replyService.want_list(sell_bno);
	...
	public int postSellUpdateLike(int sell_bno, int user_num) throws Exception {
		...
		int want_likeCheck = service.sell_likeCheck(sell_bno, user_num);
		if (want_likeCheck == 0) {
			// 좋아요 처음누름
