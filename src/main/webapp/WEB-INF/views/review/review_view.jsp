				...
				<div class="col-lg-8">
					<div class="section-title mb-0">
						...
						<p style="color: black;" class="m-0 text-uppercase font-weight-bold px-8">
							<fmt:formatDate value="${view.review_regDate}" pattern="yyyy-MM-dd" />
						</p>
					</div>
					<div class="section-title mb-1">
						<p style="color: black;" class="m-0 text-uppercase font-weight-bold px-0">${view.review_writer}</p>
						<p style="color: black;" class="m-0 text-uppercase font-weight-bold px-0">조회수 ${view.review_viewCnt}</p>
						<!-- 유저 이름과 게시글 작성자가 일치해야 게시글 수정, 삭제 가능 
							  하지만 동명이인 일시? 추후 id로 개선 필요함
						-->
						<c:if test="${member.user_name == view.review_writer}">
							<div>
								<a href="/review/review_modify?review_bno=${view.review_bno}">게시글 수정</a>
								<a href="/review/review_delete?review_bno=${view.review_bno}">게시글 삭제</a>
							</div>
						</c:if>
					</div>
							...
							<div class="text-center">
								<c:if test="${member.user_id != null and member.user_id != dto.user_id}">
									<div style="margin-right: 1px;">
										<button type="button" class="btn btn-warning" id="like_btn" onclick="review_updateLike(); return false;">추천 ${view.review_like}</button>
									</div>
								</c:if>
							</div>
						</div>
...
<script>
var review_bno = ${view.review_bno};
var user_num = ${member.user_num};

 function review_updateLike(){ 
     $.ajax({
            type : "POST",  
            url : "/review/review_updateLike",       
            dataType : "json",   
            data : {'review_bno' : review_bno, 'user_num' : user_num},
            error : function(){
               alert("통신 에러");
            },
